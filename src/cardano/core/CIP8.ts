...
import {
    ...
    resolveStakeKeyHash, resolvePaymentKeyHash, toBytes,
} from "../common/utils/index";

import type { DataSignature } from "../common/types/index";
import type { Address, PrivateKey, PublicKey } from "../core";
...
const checkAddress = (
    ...
    try {
        const keyHash = [
            resolvePaymentKeyHash(signer),
            resolveStakeKeyHash(signer)
        ]
        return keyHash.includes(publicKey.hash().to_hex());
    } catch (error) {
        return false;
    }
};
